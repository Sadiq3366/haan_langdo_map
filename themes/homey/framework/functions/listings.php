...
//            that is removed because was not standard but one clients request.
        ...
        if( $listing_id > 0 ) {
            ...
            update_post_meta($listing_id,'booking_petteren', $listing_id + 1);
            ...
            if( isset( $_POST['provience'] ) ) {
                update_post_meta( $listing_id, $prefix.'provience', sanitize_text_field( $_POST['provience'] ) );
            }

            // Listing Type
            
            if( isset( $_POST['listing_type'] ) && ( $_POST['listing_type'] != '-1' ) ) {
                $types_listings=array();
                foreach($_POST['listing_type'] as $listing_types)
                {
                   $types_listings[] = intval( $listing_types );
                }
                wp_set_object_terms( $listing_id, $types_listings , 'listing_type' );
            }
...
if(!function_exists('homey_get_listing_featured')) {
    function homey_get_listing_featured($listing_id) {
        ...
        $terms = wp_get_post_terms( $listing_id, 'listing_type', array("fields" => "names"));
          $t = '';
          if (!empty($terms)) {
        ...
        if($featured == 1) {
            ...
            if(is_singular('listing')) {
                $html_output = '<span class="label label-success label-featured">'.$type_listing.'</span>';
            } else {
                $html_output = '<span class="label-wrap top-left">
                    <span class="label label-success label-featured">'.$type_listing.'</span>
                </span>';
            }
        }
        return $html_output;
