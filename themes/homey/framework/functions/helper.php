...
if(!function_exists('homey_get_types_value')){
    function homey_get_types_value($taxonomy, $parent_taxonomy, $taxonomy_id, $prefix = " " ){

        if (!empty($parent_taxonomy)) {

            foreach ($parent_taxonomy as $term) {
                foreach($taxonomy_id as $parents)
                {
                
                    if ($parents != $term->term_id) {
                           
                    echo  '
                    <li><i class="fa fa-futbol-o"></i>
                    <span class="item-label">'.$term->name.'</span>
                    </li>
                    ';
                ...
                if (!empty($get_child_terms)) {
                    homey_get_types_value( $taxonomy, $get_child_terms, $taxonomy_id, "- ".$prefix );
                }
            }
...
if(!function_exists('homey_get_taxonomies_with_id_value')){
    function homey_get_taxonomies_with_id_value($taxonomy, $parent_taxonomy, $taxonomy_id, $prefix = " " ){
        ...
        if (!empty($parent_taxonomy)) {
            foreach ($parent_taxonomy as $term) {
                if(!empty($taxonomy_id) && $taxonomy_id > 0 )
                {
                    foreach($taxonomy_id as $parents)
                    {
                        if ($parents != $term->term_id) {
...
if(!function_exists('homey_get_taxonomies_for_type_value')){
    function homey_get_taxonomies_for_type_value( $listing_id, $taxonomy ){
        ...
        homey_get_types_value($taxonomy, $parent_taxonomy, $taxonomy_id);
    }
}
