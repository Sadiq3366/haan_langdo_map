            ...
            $.ajax({
                ...
                success: function( response ) {
                    if( response.success ) {
					    ...
					    var	$otpfield = $('.modal-login-form').find('input[name=register_number_valid]').val();
						
						if( $otpfield == null)
						{
						    $('.modal-login-form').find('.otp_field').append('<div class="form-group"><input type="number" name="register_otp" class="form-control password-input-2" placeholder="Enter OTP Number"></div>');
							$('.modal-login-form').find('.otp_field').append('<div class="form-group"><input type="hidden" name="register_number_valid" value='+ response.otp +'></div>');
						}
